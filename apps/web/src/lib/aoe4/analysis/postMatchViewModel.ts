...
export interface MetricAgeAnalysisCard {
  age: AgeAnalysisAge;
...
export interface PostMatchViewModel {
  ...
  metricCards: {
    ...
    ageAnalyses: MetricAgeAnalysisCard[];
  };
  trajectory: {
...
interface AgeAllocationDelta {
  key: AgeAllocationCategory;
  label: string;
  youDelta: number;
...
const ageAnalysisAges: AgeAnalysisAge[] = ['Dark', 'Feudal', 'Castle', 'Imperial'];

function ageDisplayName(age: AgeAnalysisAge): string {
  return age;
}
...
function ageCategoryValue(point: PoolSeriesPoint, category: AgeAllocationCategory): number {
  if (category === 'economy') return point.economic;
  if (category === 'technology') return point.research + point.advancement;
  return point.militaryCapacity + point.militaryActive + point.defensive;
}

function buildAgeAllocationDeltas(
  yourSeries: PoolSeriesPoint[],
  opponentSeries: PoolSeriesPoint[],
  start: number,
  end: number
): AgeAllocationDelta[] {
  const yourStart = pointAtOrBefore(yourSeries, start);
  const yourEnd = pointAtOrBefore(yourSeries, end);
  const opponentStart = pointAtOrBefore(opponentSeries, start);
  const opponentEnd = pointAtOrBefore(opponentSeries, end);
  const categories: Array<{ key: AgeAllocationCategory; label: string }> = [
  ...
  return categories.map(category => {
    const youDelta = ageCategoryValue(yourEnd, category.key) - ageCategoryValue(yourStart, category.key);
    const opponentDelta = ageCategoryValue(opponentEnd, category.key) - ageCategoryValue(opponentStart, category.key);
    return {
      key: category.key,
      label: category.label,
      youDelta,
      opponentDelta,
      edge: youDelta - opponentDelta,
    };
  });
}

function formatAgeEdge(delta: AgeAllocationDelta): string {
  return `${delta.label} +${Math.round(Math.abs(delta.edge))}`;
}

function similarAgeCategory(
  deltas: AgeAllocationDelta[],
  excluded: AgeAllocationDelta[]
): AgeAllocationDelta {
  const excludedKeys = new Set(excluded.map(delta => delta.key));
  const candidates = deltas.filter(delta => !excludedKeys.has(delta.key));
  return candidates
    .sort((a, b) => Math.abs(a.edge) - Math.abs(b.edge) || a.label.localeCompare(b.label))[0]
    ?? deltas[0];
}
...
function buildAgeAllocationSummary(deltas: AgeAllocationDelta[]): string {
  ...
  const hasOpponentEdge = opponentEdge.edge <= -100;
...
function lossValueDuring(events: BandItemDeltaEvent[] | undefined, start: number, end: number): number {
  return (events ?? [])
    .filter(event => event.timestamp >= start && event.timestamp <= end && event.deltaValue < 0)
    .reduce((sum, event) => sum + Math.abs(event.deltaValue), 0);
}

function buildAgeDestructionSummary(
  ...
  if (yourLosses < 1 && opponentLosses < 1) {
...
function minGatherRateInWindow(series: GatherRatePoint[], start: number, end: number): number {
  const startRate = gatherRateAtOrBefore(series, start);
  const inWindow = series
    .filter(point => point.timestamp >= start && point.timestamp <= end)
    .map(point => point.ratePerMin);
  return Math.min(startRate, ...inWindow);
}

function gatherRateDropPct(series: GatherRatePoint[], start: number, end: number): number {
  const startRate = gatherRateAtOrBefore(series, start);
  if (startRate <= 0) return 0;
  const minRate = minGatherRateInWindow(series, start, end);
  return Math.max(0, (startRate - minRate) / startRate);
}

function buildAgeConversionSummary(params: {
  age: AgeAnalysisAge;
  deltas: AgeAllocationDelta[];
  ...
  const military = params.deltas.find(delta => delta.key === 'military');
  const yourMilitaryDelta = military?.youDelta ?? 0;
  const opponentMilitaryDelta = military?.opponentDelta ?? 0;
  const yourDestroyed = lossValueDuring(params.opponentEvents, params.start, params.end);
...
function buildAgeAnalyses(params: {
  summary: GameSummary;
  youIndex: 0 | 1;
  yourSeries: PoolSeriesPoint[];
  opponentSeries: PoolSeriesPoint[];
  ...
  const ageTimes: Record<PostMatchPlayerKey, Record<AgeMarkerAge, number | null>> = {
    you: {
      Feudal: getAgeUpTime(yourPlayer.actions, 'Feudal'),
      Castle: getAgeUpTime(yourPlayer.actions, 'Castle'),
      Imperial: getAgeUpTime(yourPlayer.actions, 'Imperial'),
    },
    opponent: {
      Feudal: getAgeUpTime(opponentPlayer.actions, 'Feudal'),
      Castle: getAgeUpTime(opponentPlayer.actions, 'Castle'),
      Imperial: getAgeUpTime(opponentPlayer.actions, 'Imperial'),
    },
  };

  const nextAge: Record<AgeAnalysisAge, AgeMarkerAge | null> = {
    Dark: 'Feudal',
    Feudal: 'Castle',
    Castle: 'Imperial',
    Imperial: null,
  ...
  const cards: MetricAgeAnalysisCard[] = [];

  for (const age of ageAnalysisAges) {
    if (age !== 'Dark') {
      const yourReached = ageTimes.you[age] !== null;
    ...
    const ageStart = age === 'Dark'
      ? 0
      : Math.max(ageTimes.you[age] ?? 0, ageTimes.opponent[age] ?? 0);
    const followingAge = nextAge[age];
    const nextTimes = followingAge
      ? [ageTimes.you[followingAge], ageTimes.opponent[followingAge]].filter((value): value is number => value !== null)
      : [];
    const ageEnd = followingAge && nextTimes.length > 0
      ? Math.min(...nextTimes)
      : params.summary.duration;

    if (ageEnd <= ageStart) {
      const reason = `No shared ${age} window was available to compare.`;
      cards.push({
        age,
        startTime: null,
        endTime: null,
    ...
    const conversionSummary = buildAgeConversionSummary({
      age,
      deltas,
      yourGatherRateSeries: params.yourGatherRateSeries,
    ...
    cards.push({
      ...
      startTime: ageStart,
      endTime: ageEnd,
      timeRangeLabel: `${formatTime(ageStart)}-${formatTime(ageEnd)}`,
      gapSummary,
      allocationSummary,
      destructionSummary,
      conversionSummary,
      summary: `${gapSummary} ${allocationSummary} ${destructionSummary} ${conversionSummary}`,
    });
  }
...
export function buildPostMatchViewModel(params: {
  ...
  const ageAnalyses = buildAgeAnalyses({
    summary,
    youIndex,
    yourSeries: yourPool.series,
    opponentSeries: opponentPool.series,
    yourEvents: yourPool.bandItemDeltas,
