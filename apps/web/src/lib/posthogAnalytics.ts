...
function containsSensitiveSignatureQuery(value: string): boolean {
  return /(?:sig=|sig%3d|sig%253d)/i.test(value);
}
...
function shouldDropAnalyticsProperty(key: string): boolean {
...
function sanitizeAnalyticsProperties(properties: Record<string, unknown>): AnalyticsProperties {
